// Part to print
part="leg"; // [all:Preview Bot,head:Head,body:Body,leg:Leg,foot_left:Left Foot,foot_right:Right Foot]
...
module micro_leg(move=0) {
//SG90: 23.5x12.5x26 vs. ES9251: 18.02x7.91x19.8mm = div: 5.48x4.95x6.2mm
...
//servo dimensions (already rotated)
srvWdthX=25;
srvDpthY=12.5;
srvHghtZ=50;
srvFlngtThck=2.5;
...
bo=0; //legacy offset
  ...
  difference() {
    //the Leg-Box
    translate([-ovWdthX/2,-ovDpthY/2,0]) {
      intersection() {
        cube([ovWdthX,ovDpthY,ovHghtZ]);
        union(){
          box_chamfer([ovWdthX,ovDpthY,20],wallThick); //bottom chamfered box
            
          translate([0,0,ovHghtZ+fudge]) 
            mirror([0,0,1]) 
            box_chamfer([ovWdthX,ovDpthY,20],1.25,chvx=wallThick,chvy=wallThick); //top camfered box
            
          translate([1,1,30])
            box_chamfer2([ovWdthX-2,ovDpthY-2,7],wallThick); //trousers
        }
      }
    }
    //servo cutouts 1
    ...
    translate([0.5,0,2.6]) rotate([0,0,90]) {
      linear_extrude(3.4) 
        polygon([[-3.1,0],[-2,20],[2,20],[2.8,0]]); 
        //translate([0,0,-3.01]) cylinder(d=servo_type_ankle=="mg90s"?7.8:7.4,h=8,$fn=30);
            translate([0,0,-3.01]) cylinder(d=hornAxisDia,h=8,$fn=30);
    }//horn
    
    //servo cutouts 2 
    translate([-14.5-.01,0,28.5]) rotate([0,90,0]) {  
        cylinder(d=4,h=5,$fn=30); //leg axis
        rotate([0,0,90]) translate([0,0,5]) {
            cylinder(d=srvBgGearDia,h=30,$fn=30); //big Gear
            translate([0,-5.5]) cylinder(d=srvSmGearDia,h=30,$fn=20); //small Gear
            translate([-6,0]) cube([srvBgGearDia,30,30]);//gearSlideIn
        } 
    }//cutouts2
  ...
  difference() {
    ...
    *translate([8,-3,5]) cube([5,srvScrewHdDia,8]); // screw head recess
    translate([-srvWdthX/2+srvFlngtHght+srvFlngtThck-fudge/2,0,9]) rotate([0,90,0]) cylinder(d=2.2,h=wallThick+fudge,$fn=20); //drill hole
  }
}
// } //trans-move
...
//chamfered box width equal chamfers on all edges
